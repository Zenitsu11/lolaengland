...
export async function getPublicProducts():Promise<Product[]>{
  ...
  const mapped=data.map(p=>{const urls=Array.isArray(p.image_urls) ? p.image_urls.filter((value:unknown)=>typeof value==='string' && value.trim()) as string[] : []; const front=p.image_url || urls[0] || ''; return {id:p.id,name:p.name,price:p.price,mrp:p.mrp,rating:p.rating,reviews:p.reviews,description:p.description,image_url:front,image_urls:urls,secondary_image_url:urls[1],categories:Array.isArray(p.categories)?p.categories:[],amazon:p.amazon_url,flipkart:p.flipkart_url,tone:'#f0e2e5'};});
  return mapped;
}
