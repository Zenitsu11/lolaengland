...
export default function CartPage(){
 ...
 <div className="cart-grid"><section>{items.map(i=><div className="cart-item" key={i.lineId}><img src={i.image} alt=""/><div className="cart-item-info"><h3>{i.name}</h3><p>Size: <strong>{i.size}</strong> · ₹{i.price.toLocaleString('en-IN')}</p><div className="qty"><button onClick={()=>setQuantity(i.lineId,i.quantity-1)} disabled={i.quantity<=1}><Minus/></button><b>{i.quantity}</b><button onClick={()=>setQuantity(i.lineId,i.quantity+1)}><Plus/></button></div></div><button className="remove-item" onClick={()=>remove(i.lineId)}><Trash2/></button></div>)}</section>
 <aside className="cart-summary"><p className="editorial-eyebrow">ORDER SUMMARY</p><div><span>Subtotal</span><b>₹{total.toLocaleString('en-IN')}</b></div><div><span>Shipping</span><b>{total>=799?'FREE':'Calculated at checkout'}</b></div><hr/><div className="cart-total"><span>Total</span><b>₹{total.toLocaleString('en-IN')}</b></div><Link className="checkout-pay" href="/checkout">Proceed to secure checkout →</Link><small><ShieldCheck/> UPI QR · UPI apps · Cashless checkout</small></aside></div>}
 </div></main>;
}
