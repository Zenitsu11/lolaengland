...
export async function GET(){
 ...
 const error=customers.error||orders.error||products.error||inventory.error;
 if(error)return NextResponse.json({error:error.message},{status:500});
 return NextResponse.json({customers:customers.data||[],orders:orders.data||[],products:products.data||[],inventory:inventory.data||[],generatedAt:new Date().toISOString()});
}