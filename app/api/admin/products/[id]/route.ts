...
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const allowed=['name','slug','price','mrp','rating','reviews','description','image_url','image_urls','video_urls','amazon_url','flipkart_url','featured','active','sort_order','categories'];
  ...
  if('video_urls' in payload) payload.video_urls=Array.isArray(payload.video_urls) ? payload.video_urls.map((value:unknown)=>String(value).trim()).filter(Boolean).slice(0,2) : [];
