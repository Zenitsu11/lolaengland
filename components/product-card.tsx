...
export type Product = { id:string|number; name:string; price:number; mrp:number; rating:number; reviews:number|string; tone?:string; amazon?:string; flipkart?:string; image_url?:string; description?:string };

export function ProductCard({ product }: { product: Product }) {
  return <article className="product-card">
    <div className="product-art" style={{background:product.tone || '#f0e2e5'}}><span className="pill">TRENDING</span>{product.image_url ? <img className="product-image" src={product.image_url} alt={product.name}/> : <div className="tee"><div className="tee-neck"/><div className="tee-logo">LOLA<br/><small>ENGLAND</small></div></div>}</div>
